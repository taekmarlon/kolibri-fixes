...
<script>
  ...
  export default {
    ...
    setup(props) {
      ...
      const liveSessionPolling = useTimeoutPoll(fetchLiveSessions, 2000);
      ...
      function joinLiveMeeting() {
        ...
        const roomName = getClassRoomName(props.classId, className.value);
        const directUrl = buildLiveMeetingUrl({
          roomName,
          displayName: studentName,
          subject: className.value ? `${className.value} — Live Class` : 'PHIEDU Live Class',
