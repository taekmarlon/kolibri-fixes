 ...
 */
const getLibraryFilePathOverrideScript = (0, esbuild_1.transformSync)((0, fs_1.readFileSync)(path_1.default.join(__dirname, 'loadFileOverrides.js'), {
    encoding: 'utf8'
}), { minify: true, loader: 'js' }).code;
const getContentPathOverrideScript = (0, esbuild_1.transformSync)(`H5P.getPath = function (path, contentId) {
        return path;
    };
    `, { minify: true, loader: 'js' }).code;
/**
...
class HtmlExporter {
    ...
    async findAndPrefixContentResources(model, prefix = '') {
        ...
        fileRefs.forEach((ref) => {
            ref.context.params.path = path_1.default.posix.join(prefix, ref.filePath);
        });
    ...
    async getScriptBundle(model, usedFiles, additionalScripts = []) {
        ...
        return (await (0, esbuild_1.transform)(scripts.join('\n'), {
            minify: true,
        ...
        await Promise.all(model.styles.map(async (style) => {
            ...
            (0, postcss_import_1.default)({
                resolve: (importedFile) => {
                    ...
                    // library).
                    return path_1.default.posix.join(path_1.default.dirname(style), importedFile);
                },
                ...
                plugins: [
                    ...
                    (0, postCssRemoveRedundantFontUrls_1.default)(undefined, library
                        ? (f) => {
                            usedFiles.addFile(library, path_1.default.posix.join(path_1.default.dirname(filename), f));
                        }
                        : undefined),
    ...
    async minifyCss(css) {
        try {
            return (await (0, esbuild_1.transform)(css, {
                loader: 'css',
                minify: true,
    ...
    urlInternalizer = (filename, library, editor, core, usedFiles) => async (asset) => {
        ...
        if (library) {
            const p = path_1.default.posix.join(path_1.default.dirname(filename), asset.relativePath);
            try {
