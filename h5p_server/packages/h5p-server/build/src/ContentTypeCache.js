    ...
    async downloadContentTypesFromHub() {
        ...
        try {
            ...
            throw new H5pError_1.default('error-communicating-with-hub', {
                statusCode: error.response?.status?.toString() || 'no response',
    ...
    async get(...machineNames) {
        ...
        if (!cache) {
            if (this.config.fetchingDisabled) {
                log.info('ContentTypeCache fetching is disabled. Using local installed libraries.');
            ...
            if (!cache) {
                log.info('ContentTypeCache could not be retrieved from H5P Hub. Saving empty cache to avoid blocking.');
                await this.storage.save('contentTypeCache', []);
                await this.storage.save('contentTypeCacheUpdate', Date.now());
                return [];
            }
    ...
    getLocalId() {
        ...
        if (this.getLocalIdOverride) {
            ...
            if (localId.length > maxLength) {
                throw new H5pError_1.default('error-local-id-too-long', {
            ...
            return (0, node_machine_id_1.machineIdSync)().substring(0, maxLength);
        }
    }
}
