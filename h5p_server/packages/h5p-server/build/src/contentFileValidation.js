...
exports.hasBufferData = hasBufferData;
exports.validateContent = validateContent;
exports.validateFileContent = validateFileContent;
exports.validateBufferContent = validateBufferContent;
exports.extensionMatchesDetected = extensionMatchesDetected;
...
const log = new Logger_1.default('contentFileValidation');
/**
 ...
 */
function hasBufferData(file) {
    return (file.data !== undefined && (file.data.length > 0 || !file.tempFilePath));
}
/**
...
async function validateContent(file) {
    if (hasBufferData(file)) {
        ...
        log.error(`File has no data or tempFilePath for validation: ${file.name}`);
 ...
 */
async function validateFileContent(filePath, originalFilename) {
    ...
    if (!filePath || !filePath.trim() || !path_1.default.isAbsolute(filePath)) {
        log.error(`Invalid file path provided to validateFileContent: ${filePath}`);
        throw new H5pError_1.default('upload-validation-error', {}, 400);
    }
    ...
    const fileHandle = await (0, promises_1.open)(resolvedPath, 'r');
    try {
 ...
 */
async function validateBufferContent(buffer, filename) {
 ...
 */
function validateContentBytes(data, ext, label) {
    if (!ext) {
        log.info(`File has no extension in its original filename. Rejecting file: ${label}`);
        throw new H5pError_1.default('upload-validation-error', {}, 400);
    }
    // Detect the actual file type using magic bytes
    const detectedExtensions = (0, magic_bytes_js_1.filetypeextension)([...data]).map((e) => e.replace(/^\./, ''));
    if (detectedExtensions.length > 0) {
        ...
        if (extensionMatchesDetected(ext, detectedExtensions)) {
            ...
            if (detectedExtensions.includes('txt') &&
                looksLikeXmlOrHtml(data)) {
                log.info(`File detected as text but contains XML/HTML content. Rejecting file: ${label}`);
                throw new H5pError_1.default('upload-validation-error', {}, 400);
            }
            return;
        }
        log.info(`File content mismatch: ${label} claims to be .${ext} but detected as ${detectedExtensions.join(', ')}. Rejecting file.`);
        throw new H5pError_1.default('upload-validation-error', {}, 400);
    }
    ...
    if (looksLikeXmlOrHtml(data)) {
        log.info(`File contains XML/HTML content but claims to be .${ext}. Rejecting file: ${label}`);
        throw new H5pError_1.default('upload-validation-error', {}, 400);
    }
}
/**
 ...
 * "video/mp4").
 */
function getMimeSubtype(mimeType) {
 ...
 */
function extensionMatchesDetected(claimedExt, detectedExtensions) {
    // Tier 1: direct match
    if (detectedExtensions.includes(claimedExt)) {
        return true;
    }
    // Tier 2: MIME type / subtype match
    const claimedMime = (0, mime_types_1.lookup)(claimedExt);
    if (claimedMime) {
        const claimedSubtype = getMimeSubtype(claimedMime);
        for (const det of detectedExtensions) {
            const detMime = (0, mime_types_1.lookup)(det);
            if (!detMime)
                continue;
            if (detMime === claimedMime)
                return true;
 ...
 */
function looksLikeXmlOrHtml(buffer) {
    const head = buffer
        .subarray(0, 1024)
        .toString('utf-8')
        .trimStart()
...
//# sourceMappingURL=contentFileValidation.js.map