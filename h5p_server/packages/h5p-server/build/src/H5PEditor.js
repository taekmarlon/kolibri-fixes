...
const contentFileValidation_1 = require("./contentFileValidation");
...
class H5PEditor {
    ...
    async saveContentFile(contentId, field, file, user) {
        ...
        if ((field.type === 'image' && !file.mimetype.startsWith('image/')) ||
            ...
            log.debug(`Invalid file upload: field type is ${field.type} but mimetype is ${file.mimetype}`);
        ...
        if (!file.tempFilePath && !file.data) {
            log.debug(`Invalid file upload: no data or tempFilePath provided for file ${file.name}`);
            throw new H5pError_1.default('upload-validation-error', {}, 400);
        }
        // Scan for malware
        ...
        await (0, contentFileValidation_1.validateContent)(file);
        ...
        try {
            if (file.mimetype.startsWith('image/')) {
                const imageBuffer = (0, contentFileValidation_1.hasBufferData)(file)
                    ? file.data
                    : (0, fs_1.readFileSync)(file.tempFilePath);
        ...
        let dataStream;
        if ((0, contentFileValidation_1.hasBufferData)(file)) {
     ...
     */
    async sanitizeFile(sanitizer, file) {
        if ((0, contentFileValidation_1.hasBufferData)(file)) {
            if (sanitizer.sanitizeBuffer) {
                return sanitizer.sanitizeBuffer(file);
            }
            return this.withBufferAsTempFile(file.data, file.name, async (tempFilePath) => {
                const result = await sanitizer.sanitize(tempFilePath, file.name);
                file.data = await (0, promises_1.readFile)(tempFilePath);
                return result;
            });
        }
        if (file.tempFilePath) {
     ...
     */
    async scanAllForMalware(file) {
        const runAll = (fileForPathBasedScanners) => Promise.all(this.malwareScanners.map(async (scanner) => ({
            ...(await this.scanForMalware(scanner, scanner.scanBuffer ? file : fileForPathBasedScanners)),
        ...
        const needsSharedTempFile = this.malwareScanners.some((scanner) => !scanner.scanBuffer);
     ...
     */
    async scanForMalware(scanner, file) {
        if ((0, contentFileValidation_1.hasBufferData)(file)) {
            if (scanner.scanBuffer) {
                return scanner.scanBuffer(file);
            }
            return this.withBufferAsTempFile(file.data, file.name, (tempFilePath) => scanner.scan(tempFilePath));
        }
        if (file.tempFilePath) {
     ...
     * `sanitize` methods.
     */
    async withBufferAsTempFile(data, fileName, callback) {
        ...
        }, { postfix: path_1.default.extname(fileName) || undefined });
    }
}
