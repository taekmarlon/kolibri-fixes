...
const start = async (): Promise<void> => {
    ...
    server.use((req: IRequestWithUser, res, next) => {
        const userId = (req.headers['x-kolibri-user-id'] as string) || '1';
        const userName =
            (req.headers['x-kolibri-user-name'] as string) || 'Kolibri User';
        const userEmail =
            (req.headers['x-kolibri-user-email'] as string) ||
            'user@kolibri.local';
        req.user = new User(userId, userName, userEmail);
        next();
    });
