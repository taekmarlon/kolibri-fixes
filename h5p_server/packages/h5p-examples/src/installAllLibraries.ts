...
async function main() {
    ...
    const user = new User('1', 'System Administrator', 'admin@kolibri.local');

    console.log('Fetching Content Type Cache from H5P Hub...');
    const allContentTypes = await h5pEditor.contentTypeCache.get();
    console.log(
        `Discovered ${allContentTypes.length} content types on H5P Hub.`
    );

    const installedLibsWrapped =
        await h5pEditor.libraryManager.listInstalledLibraries();
    ...
    // Prioritized list of essential and popular H5P interactive content types
    ...
    // Also include any other runnable types discovered from Hub
    for (const ct of allContentTypes) {
        if (
            !contentTypesToInstall.includes(ct.machineName) &&
            ct.machineName !== 'H5P.IFrameEmbed'
        ) {
    ...
    console.log(
        `Total content types to process: ${contentTypesToInstall.length}`
    );
    ...
    for (const machineName of contentTypesToInstall) {
        ...
        try {
            ...
            const results = await h5pEditor.installLibraryFromHub(
                machineName,
                user
            );
            console.log(
                `[INSTALLED] ${machineName} (+${results.length} dependencies)`
            );
            successCount++;
        } catch (err: any) {
            console.warn(
                `[FAILED/SKIPPED] ${machineName}: ${err.message || err}`
            );
            failCount++;
        }
    }
