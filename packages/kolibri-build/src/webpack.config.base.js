...
module.exports = ({ mode = 'development', hot = false, cache = false, transpile = false } = {}) => {
  ...
  const sassLoader = {
    ...
    options: {
      // Use webpack's resolver for ~ tilde imports (legacy node-sass behaviour)
      webpackImporter: true,
      // Use the legacy Sass JS API for Vue 2 compatibility (/deep/, CSS at root, etc.)
      api: 'legacy',
      sassOptions: {
        // Allow bare imports like 'kolibri-common/styles/definitions' to resolve from node_modules
        includePaths: [nodeModulesPath],
        // Silence deprecation warnings from legacy @import syntax in third-party packages
        quietDeps: true,
